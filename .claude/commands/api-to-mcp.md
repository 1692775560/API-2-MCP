...
5. If this project is not present locally and `api-to-mcp` is unavailable, install it with `npm install -g @taozhang123/api-to-mcp`, then run `api-to-mcp`.
...
- Run `api-to-mcp $ARGUMENTS`, where `api-to-mcp` is either `node dist/cli.js` in this repo or the globally installed `@taozhang123/api-to-mcp` CLI.
