...
5. If this project is not present locally, use the GitHub package with `npx github:1692775560/API-2-MCP`.
6. Never print API keys or bearer tokens back to the user.
7. After generation, tell the user the output path and the exact commands to run the generated MCP server.
...
- Run `api-to-mcp $ARGUMENTS`, where `api-to-mcp` is either `node dist/cli.js` in this repo or `npx github:1692775560/API-2-MCP`.
